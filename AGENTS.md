...
9. **Responses are camelCase; `@opensea/api-types` is snake_case.** Every fetcher method returns `Camelize<T>`, so a raw wire type used to annotate an SDK return value is the wrong shape at runtime, and the compiler only rejects it where the wire type has a required snake_case key somewhere in its tree. Export a camelized alias in `src/api/types.ts` for a new response, or point the caller at `Camelize<WireType>`, which the root now exports. `test/api/responseCasing.spec.ts` pins both verdicts, one with `@ts-expect-error` and one without, and the README's "Response casing" section is type-checked by `pnpm run check-doc-examples`.
